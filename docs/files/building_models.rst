...
As an example, a :math:`10kW` heat pump could refer to :math:`10kW$_{th}$` heat output or :math:`10kW$_{el}$` electricity input.
...
.. note::
    ...
    If you want to fix the ratio, the attribute ``energy_to_power_ratio`` in ``attributes.json`` can be set to anything different than ``inf``
