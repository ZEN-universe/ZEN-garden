...
.. code-block::
    ...
    def constraint_example_constraint(self):
        ...
        """
        term_product_xp = (
            self.optimization_model.variables["x"]
            * self.optimization_model.parameters.p
        )
        lhs = term_product_xp + self.optimization_model.variables["y"]
        rhs = self.optimization_model.parameters.p
        constraints = lhs <= rhs
        self.optimization_model.add_constraint(
            "constraint_example_constraint_upper", constraints
        )
        rhs = self.optimization_model.parameters.q
        constraints = lhs >= rhs
        self.optimization_model.add_constraint(
...
.. code-block::
    ...
    lhs = self.optimization_model.variables["shed_demand"]
    rhs = self.optimization_model.parameters.demand
    constraints = lhs <= rhs
    self.optimization_model.add_constraint(
...
.. code-block::
    ...
    mask = self.optimization_model.parameters.price_shed_demand != np.inf
    lhs = self.optimization_model.variables["shed_demand"]
    rhs = self.optimization_model.parameters.demand.where(mask,0.0)
    constraints = lhs <= rhs
    self.optimization_model.add_constraint("constraint_shed_demand",constraints)
...
.. code-block::
    ...
    mask_0 = self.optimization_model.parameters.price_shed_demand != 0
    mask_inf = self.optimization_model.parameters.price_shed_demand != np.inf
    lhs = self.optimization_model.variables["shed_demand"].where(mask_0)
    rhs = self.optimization_model.parameters.demand.where(mas_inf,0.0)
    constraints = lhs <= rhs
    self.optimization_model.add_constraint("constraint_shed_demand",constraints)
...
.. code-block::
    ...
    term_expanded_cost_carrier = self.optimization_model.variables["cost_carrier"].broadcast_like(times)
...
.. code-block::
    ...
    capacity = self.optimization_model.variables["capacity"].rename({"set_technologies": "set_storage_technologies", "set_location": "set_nodes"})
...
.. code-block::
    ...
    capacity = self.map_and_expand(self.optimization_model.variables["capacity"], times)
