...
res_scenario = Results("../outputs/hard_to_abate_180424/")
...
'''def energy_carrier(scenario):
    ...
    energy_carriers = ['coal', 'coal_for_cement', 'natural_gas', 'electricity', 'dry_biomass', 'biomethane', 'biomass_cement']
    inputs_grouped = inputs_grouped[inputs_grouped['carrier'].isin(energy_carriers)]
    outputs_grouped = outputs_grouped[outputs_grouped['carrier'].isin(energy_carriers)]

    inputs_final = inputs_grouped.groupby(['carrier', 'node']).sum().reset_index()
    outputs_final = outputs_grouped.groupby(['carrier', 'node']).sum().reset_index()

    fig, ax1 = plt.subplots(figsize=(12, 8))
    inputs_final.set_index('carrier').T.plot(kind='bar', ax=ax1)
    ax1.set_title(f"Energy Carriers for {scenario_name}")
    ax1.set_ylabel("Energy and Feedstock Input per Year [TWh]")

    ax2 = ax1.twinx()
    outputs_final.set_index('carrier').T.plot(kind='line', ax=ax2, color='r')
    ax2.set_ylabel("Outputs per Year [TWh]")

    plt.show()

    return inputs_final, outputs_final'''
...
def draw_wedges_on_map(folder_path, shapefile_path, year, scenario, radius_factor, figsize=(13, 15)):
    ...
    df = energy_carrier(folder_path, scenario)
    ...
    ax.set_xlim([gdf.total_bounds[0] + 4700000, gdf.total_bounds[2] - 3000000])
    ax.set_ylim([gdf.total_bounds[1] + 4600000, gdf.total_bounds[3] - 1000000])
...
def plot_outputs(folder_path,  scenario, carrier, save_file):
    ...
    smr_biomethane_hydrogen_row = df_output[
        (df_output['technology'] == 'SMR_methanol') & (df_output['carrier'] == 'hydrogen')]
    ...
    # Setze 'technology' als Index und transpose, um Jahre als Spalten zu haben
    grouped_df.set_index('technology', inplace=True)
    grouped_df = grouped_df.drop(columns=['carrier'])  # Entferne 'carrier', da nicht benötigt
    grouped_df_values = grouped_df.transpose()  # Jahre sind jetzt die Index

    # Plotte die Daten
    fig, ax = plt.subplots(figsize=(16, 10))
    bottom = np.zeros(len(grouped_df_values))
    colors = plt.cm.tab20(np.linspace(0, 1, len(grouped_df_values.columns)))

    # Iteriere durch jede Technologie
    for idx, technology in enumerate(grouped_df_values.columns):
        values = grouped_df_values[technology].values  # Werte für das Balkendiagramm
        ax.bar(grouped_df_values.index, values, bottom=bottom, color=colors[idx], label=technology, width=0.6)
        bottom += values

    ax.set_xlabel('Year')
    ax.set_ylabel('Production [TWh]' if carrier == 'hydrogen' else 'Production [Mt]')
    ax.legend(title='Technology')
    plt.show()

    if save_file:
        plt.savefig(f"{folder_path}/{scenario}_{carrier}_output.png")
...
def calc_lco(scenario, discount_rate, carrier):
    ...
    for year in range(14):  # Assuming the year columns are labeled as integers from 0 to 13
...
if __name__ == '__main__':
    ...
    for scenario in scenarios:
        for year in years:
            ...
            #draw_wedges_on_map(folder_path, shapefile_path, year, radius_factor=0.4, scenario=scenario)
    ...
    #"scenarios_140324/transport_flow.csv"
    shapefile_path = "nuts_data/NUTS_RG_20M_2021_4326.shp"
    output_data = res_scenario.get_total("flow_conversion_output").xs(scenario).reset_index()
    #"scenarios_140324/flow_conversion_output_scenario_.csv"
    csv_path_hydrogen = res_scenario.get_total("flow_conversion_output").xs(scenario).reset_index()
    #"scenarios_140324/flow_conversion_output_scenario_.csv"
    years = [0, 5, 8, 13]
