...
class Carrier(Element):
    ...
    def construct_params(cls, energy_system: EnergySystem):
        ...
        energy_system.parameters.add_parameter(name="shed_demand_price_high", data=energy_system.initialize_component(cls, "shed_demand_price_high", index_names=["set_carriers"]),
            doc='Parameter which specifies the high price to shed demand')
        # max shed demand low
        energy_system.parameters.add_parameter(name="max_shed_demand", data=energy_system.initialize_component(cls, "max_shed_demand", index_names=["set_carriers"]),
            doc='Parameter which specifies the maximum fraction of shed demand at low price')
        # max shed demand high
        energy_system.parameters.add_parameter(name="max_shed_demand_high", data=energy_system.initialize_component(cls, "max_shed_demand_high", index_names=["set_carriers"]),
...
class CarrierRules:
    ...
    def constraint_cost_shed_demand_high_rule(self,model, carrier, node, time):
    ...
    def constraint_limit_shed_demand_high_rule(self,model, carrier, node, time):
                ...
                sum(
                    sum(
                        ...
                        * params.time_steps_operation_duration[carrier, time]
                        for time in self.energy_system.encode_time_step(carrier, base_time_step, yearly=True))
