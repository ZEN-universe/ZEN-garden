...
class ConditioningCarrierRules(GenericRule):
    ...
    def constraint_nodal_energy_balance_conditioning_block(self):
        ...
        # carrier import, demand and export
        term_carrier_import = model.variables["flow_import"].to_linexpr()
        term_carrier_export = model.variables["flow_export"].to_linexpr()
        term_carrier_demand = params.demand
        ...
        for carrier in index.get_unique([0]):
            ...
            if carrier in sets["set_conditioning_carriers"]:
                ...
                if carrier in sets["set_conditioning_carrier_parents"]:
                    term_endogenous_carrier_demand.append(-1.0*model.variables["endogenous_carrier_demand"].loc[carrier])
                else:
                    term_endogenous_carrier_demand.append(1.0 * model.variables["endogenous_carrier_demand"].loc[carrier])
            else:
                # something empty with the right shape
                term_endogenous_carrier_demand.append(lp.LinearExpression(data=None, model=model))
        # merge and regroup
        term_endogenous_carrier_demand = lp.merge(*term_endogenous_carrier_demand, dim="group")
        term_endogenous_carrier_demand = self.optimization_setup.constraints.reorder_group(term_endogenous_carrier_demand, None, None, index.get_unique([0]), index_names[:1], model)
        # shed demand
        term_carrier_shed_demand = model.variables["shed_demand"].to_linexpr()
        ...
        return lhs == term_carrier_demand
