...
class Technology(Element):
    ...
    def store_input_data(self):
        ...
        self.construction_time = self.datainput.extract_attribute("constructionTime")["value"]
        # maximum diffusion rate
        self.max_diffusion_rate = self.datainput.extract_input_data("maxDiffusionRate", index_sets=["set_time_steps"],time_steps=set_time_steps_yearly)

        # add all raw time series to dict
        self.raw_time_series = {}
        self.raw_time_series["min_load"] = self.datainput.extract_input_data("minLoad",index_sets=[_set_location, "set_time_steps"],time_steps=set_base_time_steps_yearly)
        self.raw_time_series["max_load"] = self.datainput.extract_input_data("maxLoad",index_sets=[_set_location, "set_time_steps"],time_steps=set_base_time_steps_yearly)
        self.raw_time_series["opex_specific"] = self.datainput.extract_input_data("opexSpecific",index_sets=[_set_location,"set_time_steps"],time_steps=set_base_time_steps_yearly)
        ...
        self.existing_invested_capacity = self.datainput.extract_input_data("existingInvestedCapacity", index_sets=[_set_location, "set_time_steps"], time_steps=set_time_steps_yearly)
        self.lifetime_existing_technology = self.datainput.extract_lifetime_existing_technology("existingCapacity",index_sets=[_set_location,"set_existing_technologies"])
    ...
    def calculate_capex_of_existing_capacities(self,storage_energy = False):
        ...
        if self.__class__.__name__ == "StorageTechnology":
            ...
            existing_capacities = self.existing_capacity
            existing_capex = existing_capacities.to_frame().apply(
                lambda _existing_capacity: self.calculate_capex_of_single_capacity(_existing_capacity.squeeze(),_existing_capacity.name), axis=1)
    ...
    def construct_constraints(cls):
        ...
        Constraint.add_constraint(
            model,
            name="constraint_capex_total",
            index_sets= model.set_time_steps_yearly,
            rule = constraint_capex_total_rule,
        ...
        Constraint.add_constraint(
            model,
            name="constraint_opex_total",
            index_sets= model.set_time_steps_yearly,
            rule = constraint_opex_total_rule,
        ...
        Constraint.add_constraint(
            ...
            index_sets= model.set_time_steps_yearly,
            rule=constraint_carbon_emissions_technology_total_rule,
...
def constraint_technology_lifetime_rule(model, tech,capacity_type, loc, time):
    ...
    existing_capacities = Technology.get_available_existing_quantity(tech,capacity_type,loc,time,type_existing_quantity="capacity")
    return (model.capacity[tech,capacity_type, loc, time]
            == existing_capacities
            + sum(model.built_capacity[tech,capacity_type, loc, previous_time] for previous_time in Technology.get_lifetime_range(tech,time)))
...
def constraint_technology_diffusion_limit_rule(model,tech,capacity_type ,loc,time):
    ...
    if params.max_diffusion_rate[tech,time] != np.inf:
        ...
        total_capacity_knowledge = (
            sum(
                (params.existing_capacity[tech,capacity_type,loc,existing_time]
                 # add spillover from other regions
                + sum(
                    params.existing_capacity[tech, capacity_type, other_loc, existing_time] * knowledge_spillover_rate
                    for other_loc in set_locations if other_loc != loc
                )) *
            ...
            +
            sum(
                (model.built_capacity[tech, capacity_type, loc, horizon_time]
                 # add spillover from other regions
                + sum(
                    # add spillover from other regions
                    model.built_capacity[tech, capacity_type, loc, horizon_time] * knowledge_spillover_rate
                    for other_loc in set_locations if other_loc != loc
                )) *
        ...
        return (
            model.invested_capacity[tech, capacity_type, loc, time] <=
...
def constraint_capacity_factor_rule(model, tech,capacity_type, loc, time):
    ...
    # storage technology
        ...
        # if limit power
        if capacity_type == system["set_capacity_types"][0]:
            return (model.capacity[tech,capacity_type, loc, time_step_year]*params.max_load[tech,capacity_type, loc, time] >= model.carrier_flow_charge[tech, loc, time] + model.carrier_flow_discharge[tech, loc, time])
        # TODO integrate level storage here as well
        else:
            return pe.Constraint.Skip
        # if limit energy
        # else:
        #     return (model.capacity[tech,capacity_type, loc, time_step_year] * model.max_load[tech,capacity_type, loc, time] >= model.levelStorage[tech,loc,time])
