...
class ConversionTechnology(Technology):
    ...
    def store_input_data(self):
        ...
        assert self.reference_carrier[0] in (self.input_carrier + self.output_carrier), f"reference carrier {self.reference_carrier} of technology {self.name} not in input and output carriers {self.input_carrier + self.output_carrier}"
    ...
    def construct_sets(cls):
        ...
        model.set_input_carriers = pe.Set(
            model.set_conversion_technologies,
            initialize = _input_carriers,
        ...
        model.set_dependent_carriers = pe.Set(
            model.set_conversion_technologies,
            initialize = _dependent_carriers,
    ...
    def construct_vars(cls):
        ...
        Variable.add_variable(
            model,
            name="input_flow",
            index_sets= cls.create_custom_set(["set_conversion_technologies","set_input_carriers","set_nodes","set_time_steps_operation"]),
        ...
        Variable.add_variable(
            model,
            name="output_flow",
            index_sets= cls.create_custom_set(["set_conversion_technologies","set_output_carriers","set_nodes","set_time_steps_operation"]),
            domain = pe.NonNegativeReals,
            bounds = carrier_flow_bounds,
        ...
        Variable.add_variable(
            ...
            index_sets= cls.create_custom_set(["set_conversion_technologies","set_nodes","set_time_steps_yearly"]),
            domain = pe.NonNegativeReals,
        ...
        Variable.add_variable(
            model,
            name="reference_flow_approximation",
            index_sets= cls.create_custom_set(["set_conversion_technologies","set_dependent_carriers","set_nodes","set_time_steps_operation"]),
            domain = pe.NonNegativeReals,
            bounds = carrier_flow_bounds,
        ...
        Variable.add_variable(
            model,
            name="dependent_flow_approximation",
            index_sets= cls.create_custom_set(["set_conversion_technologies","set_dependent_carriers","set_nodes","set_time_steps_operation"]),
            domain = pe.NonNegativeReals,
            bounds = carrier_flow_bounds,
    ...
    def construct_constraints(cls):
        ...
        if set_pwa_capex:
            ...
            model.constraint_pwa_capex = pe.Piecewise(set_pwa_capex[0],
                model.capex_approximation,model.capacity_approximation,
                pw_pts = pwa_breakpoints,pw_constr_type = "EQ", f_rule = pwa_values,unbounded_domain_var = True, warn_domain_coverage =False,pw_repn="BIGM_BIN")
        if set_linear_capex[0]:
            # if set_linear_capex contains technologies:
            Constraint.add_constraint(
                model,
                name="constraint_linear_capex",
                index_sets= set_linear_capex,
                rule = constraint_linear_capex_rule,
        ...
        if set_pwa_conver_efficiency:
            ...
            model.constraint_pwa_conver_efficiency = pe.Piecewise(set_pwa_conver_efficiency[0],
                model.dependent_flow_approximation,model.reference_flow_approximation,
                pw_pts = pwa_breakpoints,pw_constr_type = "EQ", f_rule = pwa_values,unbounded_domain_var = True, warn_domain_coverage =False,pw_repn="BIGM_BIN")
        ...
        Constraint.add_constraint(
            model,
            name="constraint_capex_coupling",
            index_sets= cls.create_custom_set(["set_conversion_technologies","set_nodes","set_time_steps_yearly"]),
            rule = constraint_capex_coupling_rule,
            doc = "couples the real capex variables with the approximated variables")
        # capacity
        Constraint.add_constraint(
            model,
            name="constraint_capacity_coupling",
            index_sets= cls.create_custom_set(["set_conversion_technologies","set_nodes","set_time_steps_yearly"]),
            rule = constraint_capacity_coupling_rule,
        ...
        Constraint.add_constraint(
            model,
            name="constraint_reference_flow_coupling",
            ...
            rule = constraint_reference_flow_coupling_rule,
            doc = "couples the real reference flow variables with the approximated variables")
        # dependent flow coupling
        Constraint.add_constraint(
            model,
            name="constraint_dependent_flow_coupling",
            index_sets= cls.create_custom_set(["set_conversion_technologies","set_no_on_off","set_dependent_carriers","set_location","set_time_steps_operation"]),
            rule = constraint_dependent_flow_coupling_rule,
    ...
    def disjunct_on_technology_rule(cls,disjunct, tech,capacity_type, node, time):
        ...
        Constraint.add_constraint(
            disjunct,
            name=f"constraint_reference_flow_coupling_{'_'.join([str(tech),str(node),str(time)])}",
            index_sets=[
                [[tech],model.set_dependent_carriers[tech],[node],[time]],
                ["set_conversion_technologies","setDependentCarriers","set_nodes","set_time_steps_operation"]
            ],
            rule = constraint_reference_flow_coupling_rule,
            doc ="couples the real reference flow variables with the approximated variables",
        )
        ...
        Constraint.add_constraint(
                disjunct,
                name=f"constraint_dependent_flow_coupling_{'_'.join([str(tech),str(node),str(time)])}",
                index_sets=[
                    [[tech],model.set_dependent_carriers[tech],[node],[time]],
                    ["set_conversion_technologies","setDependentCarriers","set_nodes","set_time_steps_operation"]
                ],
    ...
    def disjunct_off_technology_rule(cls,disjunct, tech, capacity_type, node, time):
        ...
        disjunct.constraint_no_load = pe.Constraint(
            expr=
            sum(model.input_flow[tech,input_carrier,node,time]     for input_carrier  in model.set_input_carriers[tech]) +
            sum(model.output_flow[tech,output_carrier,node,time]   for output_carrier in model.set_output_carriers[tech])
            == 0
        )
...
def constraint_linear_conver_efficiency_rule(model,tech,dependent_carrier,node,time):
    ...
    time_step_year = EnergySystem.convert_time_step_operation2invest(tech,time)
    return(
        model.dependent_flow_approximation[tech,dependent_carrier,node,time]
        == params.conver_efficiency_specific[tech,dependent_carrier, node,time_step_year]*model.reference_flow_approximation[tech,dependent_carrier,node,time]
    )
