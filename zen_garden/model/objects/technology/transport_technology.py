...
class TransportTechnology(Technology):
    ...
    def __init__(self, tech: str, energy_system: EnergySystem):
    ...
    def store_input_data(self):
        ...
        if self.name in self.energy_system.system["set_bidirectional_transport_technologies"]:
    ...
    def construct_params(cls, energy_system: EnergySystem):
        ...
        energy_system.parameters.add_parameter(name="distance", data=energy_system.initialize_component(cls, "distance", index_names=["set_transport_technologies", "set_edges"]),
            doc='distance between two nodes for transport technologies')
        # capital cost per unit
        energy_system.parameters.add_parameter(name="capex_specific_transport",
            data=energy_system.initialize_component(cls, "capex_specific", index_names=["set_transport_technologies", "set_edges", "set_time_steps_yearly"]),
            doc='capex per unit for transport technologies')
        # capital cost per distance
        energy_system.parameters.add_parameter(name="capex_per_distance",
            data=energy_system.initialize_component(cls, 'capex_per_distance', index_names=['set_transport_technologies', "set_edges", "set_time_steps_yearly"]),
            doc='capex per distance for transport technologies')
        # carrier losses
        energy_system.parameters.add_parameter(name="loss_flow", data=energy_system.initialize_component(cls, "loss_flow", index_names=["set_transport_technologies"]),
    ...
    def construct_vars(cls, energy_system):
        ...
        energy_system.variables.add_variable(model, name="carrier_flow", index_sets=cls.create_custom_set(["set_transport_technologies", "set_edges", "set_time_steps_operation"], energy_system), domain=pe.NonNegativeReals,
            bounds=carrier_flow_bounds, doc='carrier flow through transport technology on edge i and time t')
        # loss of carrier on edge
        energy_system.variables.add_variable(model, name="carrier_loss", index_sets=cls.create_custom_set(["set_transport_technologies", "set_edges", "set_time_steps_operation"], energy_system), domain=pe.NonNegativeReals,
    ...
    def construct_constraints(cls, energy_system):
        ...
        if "enforce_selfish_behavior" in system.keys() and system["enforce_selfish_behavior"]:
            energy_system.constraints.add_constraint(model, "disjunct_selfish_behavior_no_flow",
                                      index_sets=cls.create_custom_set(["set_transport_technologies", "set_selfish_nodes"],energy_system),
                                      rule=cls.disjunct_selfish_behavior_no_flow_rule,
                                      doc="disjunct to enforce Selfish behavior no flow",
                                      constraint_class=pgdp.Disjunct)
            energy_system.constraints.add_constraint(model, "disjunct_selfish_behavior_no_shed_demand_low",
                                      index_sets=cls.create_custom_set(["set_transport_technologies", "set_selfish_nodes"],energy_system),
                                      rule=cls.disjunct_selfish_behavior_no_shed_demand_low_rule,
                                      doc="disjunct to enforce Selfish behavior no shed demand at low cost",
                                      constraint_class=pgdp.Disjunct)
            energy_system.constraints.add_constraint(model, "disjunction_selfish_behavior",
                                      index_sets=cls.create_custom_set(["set_transport_technologies", "set_selfish_nodes"],energy_system),
                                      rule=cls.disjunction_selfish_behavior_rule,
                                      doc="disjunction to enforce Selfish behavior", constraint_class=pgdp.Disjunction)
    ...
    @classmethod
    def disjunct_off_technology_rule(cls,energy_system, disjunct, tech, capacity_type, edge, time):
        """definition of disjunct constraints if technology is off"""
        model = disjunct.model()
        disjunct.constraint_no_load = pe.Constraint(expr=model.carrier_flow[tech, edge, time] == 0)

    @classmethod
    def disjunct_selfish_behavior_no_flow_rule(cls,energy_system, disjunct, tech, node):
        ...
        model = disjunct.model()
        solver = energy_system.solver()
        edges_out = energy_system.calculate_connected_edges(node, direction="out")
    ...
    @classmethod
    def disjunct_selfish_behavior_no_shed_demand_low_rule(cls,energy_system, disjunct, tech, node):
        """ definition of disjunct constraint to enforce that reducing own voluntarily shed demand is preferred over transporting to other nodes - no flow"""
        model = disjunct.model()
        solver = energy_system.get_solver()
        ...
        for conversion_technology in set_conversion_technologies:
            if reference_carrier in ConversionTechnology.get_attribute_of_specific_element(conversion_technology,
                                                                                           "input_carrier"):
                list_connected_carriers.extend(
                    ConversionTechnology.get_attribute_of_specific_element(conversion_technology, "output_carrier"))
    ...
    @classmethod
    def disjunction_selfish_behavior_rule(cls,energy_system, model, tech, node):
        """ definition that enforces Selfish behavior disjuncts """
        return ([model.disjunct_selfish_behavior_no_flow[tech, node],
                 model.disjunct_selfish_behavior_no_shed_demand_low[tech, node]])
