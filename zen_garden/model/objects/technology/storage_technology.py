...
class StorageTechnology(Technology):
    ...
    def construct_sets(cls):
        ...
        model.set_time_steps_storage_level = pe.Set(
            model.set_storage_technologies,
            initialize = cls.get_attribute_of_all_elements("set_time_steps_storage_level"),
    ...
    def construct_params(cls):
        ...
        Parameter.add_parameter(
            name ="time_steps_storage_level_duration",
        ...
        Parameter.add_parameter(
            name="self_discharge",
            data= EnergySystem.initialize_component(cls,"self_discharge",index_names=["set_storage_technologies","set_nodes"]),
    ...
    def construct_vars(cls):
        ...
        Variable.add_variable(
            ...
            index_sets= cls.create_custom_set(["set_storage_technologies","set_nodes","set_time_steps_operation"]),
        ...
        Variable.add_variable(
            model,
            name="level_charge",
            index_sets= cls.create_custom_set(["set_storage_technologies","set_nodes","set_time_steps_storage_level"]),
            domain = pe.NonNegativeReals,
    ...
    def construct_constraints(cls):
        ...
        Constraint.add_constraint(
            model,
            name="constraint_storage_level_max",
            index_sets= cls.create_custom_set(["set_storage_technologies","set_nodes","set_time_steps_storage_level"]),
            rule = constraint_storage_level_max_rule,
            doc = 'limit maximum storage level to capacity'
        ) 
        # couple storage levels
        Constraint.add_constraint(
            model,
            name="constraint_couple_storage_level",
            index_sets= cls.create_custom_set(["set_storage_technologies","set_nodes","set_time_steps_storage_level"]),
            rule = constraint_couple_storage_level_rule,
            doc = 'couple subsequent storage levels (time coupling constraints)'
        )
        # Linear Capex
        Constraint.add_constraint(
            model,
            name="constraint_storage_technology_capex",
            index_sets= cls.create_custom_set(["set_storage_technologies","set_capacity_types","set_nodes","set_time_steps_yearly"]),
            rule = constraint_storage_technology_capex_rule,
    ...
    def disjunct_off_technology_rule(cls,disjunct, tech, capacity_type, node, time):
        ...
        model = disjunct.model()
        # off charging
        disjunct.constraint_no_load_charge = pe.Constraint(
            expr=model.carrier_flow_charge[tech, node, time] == 0
        )
        # off discharging
        disjunct.constraint_no_load_discharge = pe.Constraint(
...
def constraint_couple_storage_level_rule(model, tech, node, time):
    ...
    return(
        ...
        model.carrier_flow_discharge[tech, node, element_time_step]/params.efficiency_discharge[tech,node,time_step_year])*sum((1-params.self_discharge[tech,node])**interimTimeStep for interimTimeStep in range(0,params.time_steps_storage_level_duration[tech,time]))
    )
...
def constraint_storage_technology_capex_rule(model, tech,capacity_type, node, time):
    ...
    return (model.capex[tech,capacity_type,node, time] ==
            ...
            params.capex_specific_storage[tech,capacity_type,node, time])
