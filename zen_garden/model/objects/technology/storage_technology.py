...
class StorageTechnology(Technology):
    ...
    def construct_constraints(cls):
        ...
        # initial storage level
        Constraint.add_constraint(model, name="constraint_initial_storage_level", index_sets=cls.create_custom_set(["set_storage_technologies", "set_nodes", "set_time_steps_storage_level"]),
            rule=constraint_initial_storage_level_rule, doc='sets initial storage level')
        # Linear Capex
...
def constraint_initial_storage_level_rule(model, tech, node, time):
    ...
    if params.initial_storage_level[tech,node] != np.inf:
        ...
        if time_step_end:
            element_time_step = EnergySystem.convert_time_step_energy2power(tech, time)
            invest_time_step  = EnergySystem.convert_time_step_operation2invest(tech, element_time_step)
            return(
