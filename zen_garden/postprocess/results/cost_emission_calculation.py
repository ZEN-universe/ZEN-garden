...
class CostEmissionCalculation:
    ...
    def _get_leontief_raw_totals(self, scenario_name: str) -> dict[str, pd.DataFrame]:
        ...
        for component_type in REQUIRED_COMPONENTS:
            all_components = self.r.get_component_names(
        ...
        capex = self.r.get_total("cost_capex_yearly", scenario_name=scenario_name).groupby(["technology", "location"]).sum()
        opex = self.r.get_total("cost_opex_yearly", scenario_name=scenario_name).groupby(["technology", "location"]).sum()
        flow_in_conversion = self.r.get_total("flow_conversion_input", scenario_name=scenario_name).groupby(["technology", "carrier", "node"]).sum()
        flow_out_conversion = self.r.get_total("flow_conversion_output", scenario_name=scenario_name).groupby(["technology", "carrier", "node"]).sum()
        demand = self.r.get_total("demand", scenario_name=scenario_name).groupby(["carrier", "node"]).sum()
        shed_demand = self.r.get_total("shed_demand", scenario_name=scenario_name).groupby(["carrier", "node"]).sum()
        flow_import = self.r.get_total("flow_import", scenario_name=scenario_name).groupby(["carrier", "node"]).sum()
        flow_export = self.r.get_total("flow_export", scenario_name=scenario_name).groupby(["carrier", "node"]).sum()
        cost_carrier = self.r.get_total("cost_carrier", scenario_name=scenario_name).groupby(["carrier", "node"]).sum()
        cost_shed_demand = self.r.get_total("cost_shed_demand", scenario_name=scenario_name).groupby(["carrier", "node"]).sum()
        carbon_emissions_technology = self.r.get_total("carbon_emissions_technology", scenario_name=scenario_name).groupby(["technology", "location"]).sum()
        carbon_intensity_technology = self.r.get_total("carbon_intensity_technology", scenario_name=scenario_name).groupby(["technology", "location"]).sum()
        carbon_emissions_carrier = self.r.get_total("carbon_emissions_carrier", scenario_name=scenario_name).groupby(["carrier", "node"]).sum()
        if self.transport_technologies:
            flow_transport = self.r.get_total("flow_transport", scenario_name=scenario_name).groupby(["technology", "edge"]).sum()
            flow_transport_loss = self.r.get_total("flow_transport_loss", scenario_name=scenario_name).groupby(["technology", "edge"]).sum()
        else:
            empty_index = pd.MultiIndex.from_arrays([[], []], names=["technology", "edge"])
            flow_transport = pd.DataFrame(index=empty_index, columns=self.r.get_years(scenario_name=scenario_name))
            flow_transport_loss = pd.DataFrame(index=empty_index, columns=self.r.get_years(scenario_name=scenario_name))
        if self.storage_technologies:
            flow_storage_charge = self.r.get_total("flow_storage_charge", scenario_name=scenario_name).groupby(["technology", "node"]).sum()
            flow_storage_discharge = self.r.get_total("flow_storage_discharge", scenario_name=scenario_name).groupby(["technology", "node"]).sum()
        else:
            empty_index = pd.MultiIndex.from_arrays([[], []], names=["technology", "node"])
            flow_storage_charge = pd.DataFrame(index=empty_index, columns=self.r.get_years(scenario_name=scenario_name))
            flow_storage_discharge = pd.DataFrame(index=empty_index, columns=self.r.get_years(scenario_name=scenario_name))
        return {
            "capex": capex,
            "opex": opex,
    ...
    def _build_leontief_X(
        ...
        """
        idx = sector_index
        demand_served = (raw["demand"] - 
                         raw["shed_demand"].reindex(raw["demand"].index) + 
                         raw["flow_export"].reindex(raw["demand"].index)).fillna(0).reindex(idx).fillna(0)
        flow_in_conversion_local = raw["flow_in_conversion"].groupby(level=["carrier", "node"]).sum().reindex(idx).fillna(0)
        flow_out_conversion_local = raw["flow_out_conversion"].groupby(level=["carrier", "node"]).sum().reindex(idx).fillna(0)
        # export = raw["flow_export"].reindex(idx).fillna(0)
        imp = raw["flow_import"].reindex(idx).fillna(0)
        storage_charge = self._leontief_storage_by_carrier_node(raw["flow_storage_charge"], ref_carrier).reindex(idx).fillna(0)
        storage_discharge = self._leontief_storage_by_carrier_node(raw["flow_storage_discharge"], ref_carrier).reindex(idx).fillna(0)
        transport_out_gross, transport_in_net = self._leontief_transport_by_carrier_node(
            raw["flow_transport"], raw["flow_transport_loss"], nodes_on_edges, ref_carrier)
        transport_out_gross = transport_out_gross.reindex(idx).fillna(0)
        transport_in_net = transport_in_net.reindex(idx).fillna(0)

        X_use = demand_served + flow_in_conversion_local + transport_out_gross + storage_charge
        X_supply = flow_out_conversion_local + transport_in_net + imp + storage_discharge

        emissions_tech_pos = raw["carbon_emissions_technology"].clip(lower=0).sum().to_frame(name="emissions").T
        emissions_carrier_pos = raw["carbon_emissions_carrier"].clip(lower=0).sum().to_frame(name="emissions").T
        ...
        X_supply.loc[("emissions", "global")] = emissions_pos.loc[("emissions", "global")]

        abs_diff = (X_use - X_supply).abs()
        tolerance = atol + rtol * X_use.abs()
        bad = abs_diff[(abs_diff > tolerance).any(axis=1)]
        if len(bad) > 0:
            print(f"WARNING: Leontief X use-side/supply-side self-test mismatch for {len(bad)} sector(s) "
                  f"(max abs diff {abs_diff.to_numpy().max():.3g}, tol=atol+rtol*|X|). This indicates a bug in "
                  f"this aggregation (missed flow category, sign error, uncollapsed 'mf' index, ...), since "
                  f"the nodal energy balance constraint guarantees these match. First offenders: {list(bad.index[:10])}")
        return X_use, demand_served

    def _get_or_build_leontief_systems(self, scenario_name, overwrite=False, is_cost=True):
        """ builds (or loads from an in-memory/disk cache) the Leontief system for
        every optimization year of a scenario. Cached by scenario because
        assembling A/V_comp is carrier-independent and is the expensive part; solving
        one more carrier/single_tech/spatial query against an already-built system is
        cheap. Returns dict: step -> system dict (as built by _build_leontief_cost_year_system). """
        ...
        if self.path is not None:
            cache_file = self.path / f"{self.r.name}_{scenario_name}_{str_type}_leontief_system.pickle"
        ...
        ref_carrier = self.r.get_df(
            ...
            scenario_name=scenario_name).squeeze()
        nodes_on_edges = self.r.get_df("set_nodes_on_edges", scenario_name=scenario_name).str.split(",", expand=True)
        raw = self._get_leontief_raw_totals(scenario_name)
        sector_index = self._build_leontief_sector_index(raw)
        X, demand_served = self._build_leontief_X(raw, sector_index, nodes_on_edges, ref_carrier)
        
        input_carriers = self.r.get_df("set_input_carriers", scenario_name=scenario_name)
        output_carriers = self.r.get_df("set_output_carriers", scenario_name=scenario_name)
        ...
        for year in self.r.get_years(scenario_name=scenario_name):
            ...
            sys_step = fkt(
                year, raw, ref_carrier, input_carriers, output_carriers, set_nodes,nodes_on_edges, sector_index, X[year], demand_served[year])
    ...
    def _leontief_solve_targets(self, sys_step, target_sectors, is_cost=True):
        ...
        `value_key` selects which per-unit direct-value matrix in sys_step to apply
        (default "v_matrix" for cost; pass "e_matrix" for the emissions system). """
        if is_cost:
            value_key = "v_matrix"
        else:
            value_key = "e_matrix"
    ...
    def _leontief_assemble_frame(self, contrib, comp_to_pos, target_sectors, spatially_resolved, is_cost):
        """ turns a (component, target) contribution array into a (component, cost_type,
        origin_node[, node]) indexed Series for one year. `origin_node` is WHERE that
        component's cost originates (e.g. where a technology is installed, or where a
        carrier is imported) -- always present, so e.g. "natural gas imported at node A"
        and "at node B" remain distinguishable contributors even after being embedded
        into a target sector's cost. `node` (only present when spatially_resolved=True)
        is the separate, TARGET location being reported on -- where that cost ends up.
        `value_type_name` labels the middle index level (default "cost_type"; pass
        "emission_type" for the emissions system so the output self-documents). """
        if is_cost:
            value_type_name = "cost_type"
        else:
            value_type_name = "emission_type"

        comp_index = pd.MultiIndex.from_tuples(list(comp_to_pos.keys()), names=["component", value_type_name, "origin_node"])
        ...
        if spatially_resolved:
            ...
            out = out.reorder_levels(["carrier", "node","component", value_type_name, "origin_node"]).sort_index()
        else:
            out = out.T.groupby(level="carrier").sum().T
            out = out.stack("carrier",future_stack=True)
            out = out.reorder_levels(["carrier","component", value_type_name, "origin_node"]).sort_index()
            out = out.groupby(level=["carrier","component", value_type_name]).sum()

        return out[out != 0]

    
    def _build_leontief_cost_year_system(self, step, raw, ref_carrier, input_carriers, output_carriers, set_nodes, nodes_on_edges,
                                     sector_index, X_year, demand_served_year, eps=1e-6):
        ...
        comp_to_pos = {}
        tech_flow_in_conversion = {}  # (tech, input_carrier, node) -> flow value, kept for single_tech isolation
        ...
        def add_a(sector_in, sector_out, value):
            if value == 0 or sector_in not in sector_to_pos or sector_out not in sector_to_pos:
                return
        ...
        for tech in self.conversion_technologies:
            ...
            if len(c_out) == 1:
                ...
                raise NotImplementedError(
                    ...
                    f" with multiple output carriers {c_out} that does "
                    f"not include its reference carrier {c_ref} or that is a carbon sink.")
            ...
            for node in set_nodes:
                ...
                if key in raw["carbon_emissions_technology"].index:
                    if raw["carbon_emissions_technology"].loc[key, step] < 0:
                        continue
                    add_a(("emissions", "global"), sector_out, raw["carbon_emissions_technology"].loc[key, step] / x_out)
        # --- storage technologies: direct capex/opex ---
        # --- add A entries for charge flow, which is the only flow that creates a cross-node dependency ---
        for tech in self.storage_technologies:
            ...
            for node in set_nodes:
                ...
                if key in raw["flow_storage_charge"].index:
                    ...
                    add_a((c_ref, node), (c_ref, node), flow_charge / X_active.loc[(c_ref, node)])
                if key not in raw["capex"].index:
                    continue
                sector = (c_ref, node)
        ...
        for tech in self.transport_technologies:
            ...
            for edge in nodes_on_edges.index:
                ...
                capex_val, opex_val = raw["capex"].loc[key, step], raw["opex"].loc[key, step]
                ...
                flow_key = (tech, edge)
                if flow_key not in raw["flow_transport"].index or sector_to not in sector_to_pos:
                    continue
        ...
        for sector in active_sectors:
            ...
            if ck in raw["carbon_emissions_carrier"].index:
                add_a(("emissions", "global"), sector, raw["carbon_emissions_carrier"].loc[ck, step] / x_active)
        ...
        if min_pivot < 1e-8:
            print(f"WARNING: Leontief system for year step {step} is near-singular "
                  f"(smallest pivot {min_pivot:.3g}) -- results may be unreliable.")

        demand_from_AX = X_active - A.dot(X_active)
        diff_demand = (demand_served_year.reindex_like(demand_from_AX) - demand_from_AX).abs()
        eps_diff = 1e-3
        assert (diff_demand <= eps_diff).all(), f"Leontief demand self-test failed for year step {step}: max abs diff {diff_demand.max():.3g} > {eps_diff}. This indicates a bug in the A-matrix assembly."
        return {
            "sector_to_pos": sector_to_pos, "pos_to_sector": active_sectors,
            "comp_to_pos": comp_to_pos, "X": X_active, "A": A, "V_comp": V_comp,
            "v_matrix": v_matrix, "lu": lu, "tech_flow_in_conversion": tech_flow_in_conversion,
            "demand_served": demand_served_year,
        }
    
    def _build_leontief_emissions_year_system(self, step, raw, ref_carrier,input_carriers, output_carriers,set_nodes, nodes_on_edges,
                                               sector_index, X_year, demand_served_year, eps=1e-6):
        """ Physical-only counterpart to _build_leontief_cost_year_system, for embodied-
        emissions tracing. Reuses the identical physical-flow technical coefficients
        (conversion input->output, storage charge self-loop, transport edge, and the
        pure-sink self-consumption redirect to ("emissions","global"), kept because it
        is a structural fix for the carbon_storage-type singularity, not a cost-
        specific mechanism) -- but OMITS the two "emissions as a priced input" edges
        the cost system's A adds, since those encode a carbon-price pass-through with
        no physical-flow meaning and would make embodied-emissions results spuriously
        depend on unrelated system-wide emissions for any carrier whose chain touches
        carbon capture/storage. ("emissions","global") is therefore a pure sink here:
        things flow into it, nothing propagates back out. Builds E_comp/e_matrix
        (direct technology/carrier emissions, mirroring V_comp/v_matrix) instead of
        cost. """
        active_sectors = [s for s in sector_index if X_year.loc[s] > eps]
        sector_to_pos = {s: i for i, s in enumerate(active_sectors)}
        S = len(active_sectors)
        ...
        for tech in self.conversion_technologies:
            ...
            if len(c_out) == 1:
                ...
                raise NotImplementedError(
                    f"Leontief emissions calculation not implemented for technology {tech}"
                    f" with multiple output carriers {c_out} that does "
                    f"not include its reference carrier {c_ref} or that is a carbon sink.")
            ...
            for node in set_nodes:
                ...
                if sector_out not in sector_to_pos:
                    continue
                if key in raw["carbon_emissions_technology"].index:
                    add_e(tech, "process", sector_out, raw["carbon_emissions_technology"].loc[key, step])
                x_out = X_active.loc[sector_out]
                for c_in in in_cs:
                    fi_key = (tech, c_in, node)
                    if fi_key not in raw["flow_in_conversion"].index:
        ...
        # --- storage technologies: same self-loop as cost; no direct emissions booked ---
        for tech in self.storage_technologies:
            if tech not in ref_carrier.index:
                continue
            c_ref = ref_carrier.loc[tech]
            for node in set_nodes:
                ...
                if sector not in sector_to_pos:
                    continue
                add_e(tech, "storage", sector, raw["carbon_emissions_technology"].loc[key, step])
        # --- transport technologies: same gross-flow edge as cost; no direct emissions ---
        for tech in self.transport_technologies:
            if tech not in ref_carrier.index:
                continue
            c_ref = ref_carrier.loc[tech]
            for edge in nodes_on_edges.index:
                ...
                n_from, n_to = nodes_on_edges.loc[edge, 0], nodes_on_edges.loc[edge, 1]
                sector_from, sector_to = (c_ref, n_from), (c_ref, n_to)
                add_e(tech, "transport", sector_from, raw["carbon_emissions_technology"].loc[key, step]/2)
                add_e(tech, "transport", sector_to, raw["carbon_emissions_technology"].loc[key, step]/2)
                flow_key = (tech, edge)
        ...
        for sector in active_sectors:
            ...
            if sector in raw["carbon_emissions_carrier"].index:
                add_e(c, "fuel", sector, raw["carbon_emissions_carrier"].loc[sector, step])
                add_a(("emissions", "global"), sector, raw["carbon_emissions_carrier"].loc[sector, step] / X_active.loc[sector])
        ...
        min_pivot = np.abs(lu.U.diagonal()).min() if S > 0 else np.inf
        if min_pivot < 1e-8:
            print(f"WARNING: Leontief emissions system for year step {step} is near-singular "
                  f"(smallest pivot {min_pivot:.3g}) -- results may be unreliable.")
        ...
        return {
            "sector_to_pos": sector_to_pos, "pos_to_sector": active_sectors,
            "comp_to_pos": comp_to_pos, "X": X_active, "A": A, "E_comp": E_comp,
            "e_matrix": e_matrix, "lu": lu, "tech_flow_in_conversion": tech_flow_in_conversion,
            "tech_sector_out_preset": tech_sector_out_preset, "demand_served": demand_served_year,
        }
    ...
    def _leontief_storage_by_carrier_node(
        ...
        out = flow_df.set_axis(pd.MultiIndex.from_arrays([carriers, nodes], names=["carrier", "node"]))
    ...
    def _leontief_transport_by_carrier_node(
        ...
        return (gross_out.groupby(level=["carrier", "node"]).sum(), 
    ...
    def _build_leontief_sector_index(raw: dict[str, pd.DataFrame]) -> pd.MultiIndex:
        ...
        idx = idx.union(raw["flow_in_conversion"].groupby(level=["carrier", "node"]).sum().index)
        idx = idx.union(raw["flow_out_conversion"].groupby(level=["carrier", "node"]).sum().index)
        emissions_idx = pd.MultiIndex.from_tuples([("emissions", "global")], names=["carrier", "node"])
    ...
    def calculate_leontief_data(
        ...
        Returns:
            DataFrame: Sectoral data of the scenario
        """
        if mode == "relative" and not spatially_resolved:
            raise ValueError("Relative production data can only be returned "
        ...
        for year in self.r.get_years(scenario_name=scenario_name):
            ...
            if carrier is None:
                ...
                target_sectors = [
                    (c, n) for c in target_qty.index for n in 
                    self.r.get_system(scenario_name=scenario_name).set_nodes 
            ...
            direct = sys_step[matrix_key][:, [
                sys_step["sector_to_pos"][s] for s in target_sectors]].toarray()
