...
class TechnologyInstallation(GenericVariable):
    ...
    def get_mask(cls, model_constructor, index_sets):
        ...
        mask = xr.DataArray(
            False,
            coords=[
                optimization_model.lp_model.variables.coords["set_years"],
                optimization_model.lp_model.variables.coords["set_technologies"],
                optimization_model.lp_model.variables.coords["set_capacity_types"],
                optimization_model.lp_model.variables.coords["set_location"],
            ],
        )
        technologies = list(optimization_model.sets["set_transport_technologies"])
        if technologies:
            edges = list(optimization_model.sets["set_edges"])
            sub_mask = (
                optimization_model.parameters.distance.loc[technologies, edges]
                * optimization_model.parameters.capex_per_distance_transport.loc[
                    technologies, edges
                ]
                != 0
            ).rename(
        ...
        mask |= optimization_model.parameters.capacity_addition_min.notnull() & (
            optimization_model.parameters.capacity_addition_min != 0
        )
        index_values, index_names = index_sets
        index = MultiIndexHelper(index_values, index_names)
        sub_mask = (
            optimization_model.parameters.capacity_addition_max.notnull()
            & (optimization_model.parameters.capacity_addition_max != np.inf)
            & (optimization_model.parameters.capacity_addition_max != 0)
        )
